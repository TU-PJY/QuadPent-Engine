...
void SDK::SDK_UITool::ClampMatrixToCorner(int EdgePosition, glm::mat4& Matrix, float Width, float Height, float GapX, float GapY) {
	switch (EdgePosition) {
	case EDGE_LEFT_DOWN:
		Matrix = translate(Matrix, glm::vec3(SDK::WindowRect.LeftX + Width / 2.0 + GapX, SDK::WindowRect.LeftY + Height / 2.0 + GapY, 0.0));
		break;

	case EDGE_LEFT_UP:
		Matrix = translate(Matrix, glm::vec3(SDK::WindowRect.LeftX + Width / 2.0 + GapX, SDK::WindowRect.RightY - Height / 2.0 - GapY, 0.0));
		break;

	case EDGE_RIGHT_DOWN:
		Matrix = translate(Matrix, glm::vec3(SDK::WindowRect.RightX - Width / 2.0 - GapX, SDK::WindowRect.LeftY + Height / 2.0 + GapY, 0.0));
		break;

	case EDGE_RIGHT_UP:
		Matrix = translate(Matrix, glm::vec3(SDK::WindowRect.RightX - Width / 2.0 - GapX, SDK::WindowRect.RightY - Height / 2.0 - GapY, 0.0));
		break;
	}
}

void SDK::SDK_UITool::ClampMatrixToPosition(glm::mat4& Matrix, float PositionX, float PositionY, float Width, float Height, float GapX, float GapY) {
